...
CREATE TABLE `ltc_security`.`organize_expert` (
  ...
  `on_job` TINYINT(1) NOT NULL COMMENT '在职状态',
  `ID_number` VARCHAR(45) NOT NULL COMMENT '身份证',
  `gender` TINYINT(1) NULL COMMENT '性别',
  `position` VARCHAR(45) NULL COMMENT '职位',
  `title` VARCHAR(45) NULL COMMENT '职称',
  `personal_phone` int(11) NOT NULL COMMENT '私人电话',
  `education` VARCHAR(45) COMMENT '学历',
  `degree` VARCHAR(45) COMMENT '学位',
  `employee_number` VARCHAR(45) NULL COMMENT '员工号',
  `working_age` int(11) NULL COMMENT '工龄',
  `certificate` VARCHAR(45) NULL COMMENT '证书',
  `certificate_number` VARCHAR(45) NULL COMMENT '证书编号',
  `comment` VARCHAR(255) NULL,
  `evaluate_organize_id` int(11) NOT NULL COMMENT '机构Id',
  `active` tinyint(1) DEFAULT 1,
  `created_at` DATETIME(6) NOT NULL,
  `modified_at` DATETIME(6) NOT NULL,
  `created_by` int(11) NOT NULL,
